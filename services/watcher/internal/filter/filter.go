...
//     is NOT named FORM.md — lowercase-named .md files are private notes
//   - Visible: FORM.md at any depth (always read, even if other rules would hide it)
...
func IsVisible(path string) bool {
	...
	// FORM.md (and *.FORM.md variants) are always visible regardless of other rules.
	if IsFormFile(base) {
		return true
	}
...
// IsFormFile returns true if path is a FORM.md file at any depth.
// This includes files named exactly "FORM.md" and files whose basename
// ends with ".FORM.md" (e.g. "Agents.FORM.md") — these are sub-instruction
// files that the user may name with a qualifying prefix.
func IsFormFile(path string) bool {
	base := filepath.Base(filepath.ToSlash(path))
	return base == "FORM.md" || strings.HasSuffix(base, ".FORM.md")
}
